    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        usersSnapshot.forEach((doc) => {
          ...
          if (
            userData.answers &&
            userData.invitationCode === currentAdminCode
          ) {
  ...
  const handleExportToExcel = () => {
    try {
      ...
      const excelData = users.map((user, index) => {
        ...
        const personalScore2 = user.answers2
          ? getScore(
              user.answers2,
              user.testResults2?.personal?.score,
              "personal"
            )
          : "N/A";
        const socialScore2 = user.answers2
          ? getScore(user.answers2, user.testResults2?.social?.score, "social")
          : "N/A";
        const academicoScore2 = user.answers2
          ? getScore(
              user.answers2,
              user.testResults2?.academico?.score,
              "academico"
            )
          : "N/A";
        const fisicoScore2 = user.answers2
          ? getScore(user.answers2, user.testResults2?.fisico?.score, "fisico")
          : "N/A";

        const autoestima =
          personalScore + socialScore + academicoScore + fisicoScore;
        const autoestima2 = user.answers2
          ? (typeof personalScore2 === "number" ? personalScore2 : 0) +
            ...
            (typeof fisicoScore2 === "number" ? fisicoScore2 : 0)
          : "N/A";
      ...
      const colWidths = {
        ...
        Ciclo: 10,
        Tiempo: 15,
        ...Array.from({ length: 30 }, (_, i) => ({ [`P${i + 1}`]: 6 })).reduce(
          (acc, curr) => ({ ...acc, ...curr }),
          {}
        ),
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {users.map((user, index) => {
              ...
              const socialScore2 = user.answers2
                ? getScore(
                    user.answers2,
                    user.testResults2?.social?.score,
                    "social"
                  )
                : "N/A";
              const academicoScore2 = user.answers2
              ...
              const fisicoScore2 = user.answers2
                ? getScore(
                    user.answers2,
                    user.testResults2?.fisico?.score,
                    "fisico"
                  )
                : "N/A";
              ...
              return (
                <tr key={index} className={index % 2 === 0 ? "bg-gray-50" : ""}>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    {formatTime(user.testDuration) || "N/A"}
                  </td>
                  {Array.from({ length: 30 }, (_, i) => (
